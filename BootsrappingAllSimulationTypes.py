...
class BootstrappingPerExperiment:
        ...
        for filename in weighted_files:
            if filename.endswith('.csv'):
                ...
                sum_of_totals = entire_df.loc[:, 'CTRn=1'].values + \
                                ...
                                entire_df.loc[:, 'CTRn=4'].values
                single_file_data_normalized = single_file_col_data / sum_of_totals
                self.weighted_simulations_data_points[idx] = single_file_data_normalized
                idx += 1
    ...
    def calc_p_values_per_frame(self):
        for frame_idx, exp_frame_dist in enumerate(self.exp_data_points_normalized):
            ...
            num_of_equal_or_above = len(frame_data_from_simulations[frame_data_from_simulations >= exp_frame_dist])
            self.p_values_for_random_per_frame[
    ...
    def plot_col_data_with_p_values(self):
        ...
        coding = 'gh'
        sim_data = self.weighted_simulations_dist_averages
        ax.plot(sim_data, coding)
        lines_and_legends[
            '{} Simulation Distribution Averages'.format(self.type_of_simulation[0].capitalize())] = coding
        # plot simulation random dist averages
        coding = 'gx'
        sim_data = self.random_simulations_dist_averages
        ax.plot(sim_data, coding)
        ...
        coding = 'b^'
        pval = self.p_values_for_random_per_frame
        ax.plot(pval, coding)
        ...
        ax.set_title('EXP:{0}\nBootstrap P-Values with each type of policy\nReplications:{1}'.format(self.exp_prefix, self.number_of_replications))
        ...
        plt.show()
        path_to_plot = os.sep.join([MAIN_DATA_DIRECTORY_PATH, self.exp_prefix, 'plots', '{0}_{1}_{2}.png'.format(exp_prefix, 'all_policies', 'pvalues')])
